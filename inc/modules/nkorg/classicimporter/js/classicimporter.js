...
fpcm.nkorg_classicimporter = {

    init: function() {
        
        if (window.fpcmClassicImporterDoImport === undefined) {
            return false;
        }

        fpcm.ui.setFocus('fpcm2_path');

        jQuery('.fpcm-tabs-accordion-importer').accordion({
            disabled: fpcmClassicImporterDoImport ? false : true,
            header: 'h2',
            heightStyle: 'content'
        });
        ...
        jQuery('.fpcm-classicimporter-importstart').click(function() {
            ...
            fpcm.ajax.post('nkorg/classicimporter/' + ajaxAction, {
                data: {
                    path: jQuery('#fpcm2_path').val()
                },
                execDone: function() {
                    
                    var result = fpcm.ajax.getResult();
                    ...
                    switch (fn) {
                        ...
                        case 'btnSystemReset' :
                            ...
                            break;
                        case 'btnStartImportUploads' :                    
                            fpcm.classicimporter.importUploads(result, obj, openButton);
                            break;
                        case 'btnStartImportArticles' :                    
                            fpcm.classicimporter.importArticles(result, obj, openButton);
                            break;
                        default :
    ...
    importArticles: function(result, obj, openButton){
    ...
    importUploads: function(result, obj, openButton){
